...
The remaining platform work is adapter glue: retrieving submission files, running extraction, storing reports, and returning editor-facing or author-facing summaries. The Rust contract already models OJS, arXiv submit-ce, arXiv submission-core, generic webhooks, ScholarOne, Editorial Manager, eJournalPress, and Manuscript Manager as screening platform targets. The `provider.arxiv` lane supplies metadata evidence only; the journal adapters do not claim canonical CSL writeback or upstream arXiv acceptance.
