...
| Marketplace submission evidence | Should | 69, 70 | Host marketplaces and directories record accepted, prepared, or deferred status with version/date/install metadata where applicable, with a repeatable refresh cadence. | Prepared metadata is not public marketplace acceptance. |
| Submission requirements contracts | Should | 72 | External registry, marketplace, plugin, client, and upstream submission requirements are searched and recorded before any outward-facing submission. | Do not create issues, pull requests, marketplace listings, or registry entries without explicit approval and evidence gates. |
| arXiv upstream submission contracts | Should | 78, 79, 80, 81 | arXiv `submit-ce` and `arxiv-submission-core` requirements, maturity, stability, testing, and submission evidence are tracked separately. | Do not claim upstream arXiv acceptance from local fixtures, prepared packets, open issues, or unmerged pull requests. |
| Self-improving submission readiness | Should | 82 | Submission readiness inventory, CI workflow, and repo-health gates stay machine-readable and policy-tested. | Do not change external-submission claims unless evidence gates, blockers, and the at-least-9.5 repo-health target pass. |
