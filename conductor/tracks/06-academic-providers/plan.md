...
1. Define the provider contract and normalized candidate response model using Crossref as the first implementation target.
2. Implement Crossref lookup by DOI and bibliographic query, including provenance fields, confidence inputs, and fixture-backed parsing tests.
3. Add DOI resolution as a separate validation provider that records DOI reachability, redirect target, and resolution errors without treating reachability as bibliographic truth.
