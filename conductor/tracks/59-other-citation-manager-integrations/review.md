...
4. **Fixture-backed proof exists for the implemented paths.** Zotero has
   fixture-backed preview/apply/audit evidence under `fixtures/providers/zotero/`.
   EndNote has ENW/RIS golden fixtures under `fixtures/export/`. Other managers
   remain explicitly deferred or file-handoff-only.
...
| Gap | Impact |
|-----|--------|
| ENW/RIS reparse tests | Closed for current handoff: golden fixtures prove deterministic EndNote outputs; richer importer round-trip remains future work |
...
| Fixture directories for deferred managers | Zotero and EndNote are covered; Mendeley/Paperpile/JabRef/RefWorks/Citavi/Papers remain intentionally manifest-free or file-handoff-only |
| Missing plugin manifests (6 managers) | Only Zotero and EndNote have manifests |

## Recommendations

1. **Maintain deferral decisions** for all non-Zotero/non-EndNote managers. Document in the decision table that:
...
2. **Create plugin manifests** for Mendeley and JabRef if implementation starts. For now, keep deferred managers manifest-free.

3. **Keep docs explicit** about which managers have implementation vs. which are deferred.
