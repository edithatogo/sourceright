...
2. Establish shared export invariants. Completed in `src/export.rs` with stable record ordering and clean-output boundary checks.
...
3. Implement YAML first as the transparent diagnostic baseline. Completed as `references.yaml`.
...
4. Implement RIS and BibLaTeX as citation-manager exports. Completed as `references.ris` and `references.bib`.
...
5. Implement ENW and XML. Completed as `references.enw` and `references.xml`.
...
6. Add validation gates. Completed with unit tests for filenames, record counts, clean boundary, and format structure.
