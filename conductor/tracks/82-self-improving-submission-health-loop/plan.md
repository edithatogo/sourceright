...
- [x] Audit all 10 submission surfaces (tracks 73–81, plus this track's own health loop) and capture per-surface:
...
- [x] Identify the health score formula: `health_score = (sum of surface health_contributions / surface_count) * 10.0`, where each surface contributes `gates_passed / total_gates` as a float.
- [x] Identify surface coverage requirement: all 10 surfaces must be present in the inventory with non-zero total gates.
- [x] Document inventory schema and update cadence in this track's spec.

## Phase 2: Lock spec [x]

- [x] Formalise `submission-inventory.json` schema (defined in spec.md Data contracts section).
- [x] Formalise health score formula and surface coverage rule.
- [x] Formalise CI gate: `submission_health_policy.rs` asserts health >= 9.5 when `SOURCERIGHT_CLAIM_GATE=1`.
- [x] Formalise claim boundary: "health-monitored" is not "fully-automated".
- [x] Draft and lock spec.md, plan.md, and test-matrix.md.

## Phase 3: Implement — submission-inventory.json [x]

- [x] Create `conductor/submission-inventory.json` with initial readiness matrix for all 10 surfaces.
- [x] Populate each surface entry with:
  ...
  - Last updated date.
- [x] Compute initial health score.
- [x] Validate `submission-inventory.json` against its schema.

## Phase 4: Implement — check-submission-readiness.ps1 [x]

- [x] Create `scripts/check-submission-readiness.ps1` that:
...
- [x] Run script manually to verify it produces correct output with current inventory.
- [x] Ensure script is callable from CI workflow (does not require interactive input).

## Phase 5: Implement — submission_health_policy.rs [x]

- [x] Create `tests/submission_health_policy.rs` that:
  - Reads `conductor/submission-inventory.json`.
...
- [ ] Run `cargo test --test submission_health_policy` to verify compilation and default (ungated) pass (blocked by MSVC linker in this environment; passes on CI GNU target).
