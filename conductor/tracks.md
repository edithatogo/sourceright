...
| 73 MCP directory submission hardening | completed | Mature Official MCP Registry refresh, Smithery, and Glama submission evidence with bundle, metadata, smoke, and listing gates. Server-card and listing evidence are recorded; green downstream scan and Glama callable-tool rescan remain external residuals. |
| 74 citation-manager publication hardening | completed | Harden Zotero and EndNote publication paths with package decisions, smoke proof, import/export checks, and exact plugin-claim boundaries. Evidence level is hardened local package; external publication remains unclaimed. |
| 75 journal-platform publication hardening | completed | Mature OJS/PKP package submission and keep arXiv upstream work on separate granular tracks. Gallery search shows no match; OJS plugin skeleton and local evidence are present; external acceptance remains unclaimed. |
| 76 AI client extension publication hardening | completed | Define Claude, Codex, Copilot, Gemini CLI, and Qwen CLI extension/package paths or no-package decisions. Package decisions contract, transcript smoke, and docs updates defined. External acceptance remains separately bounded. |
| 77 VS Code and Open VSX publication hardening | completed | Hardened local VSIX package, Workspace Trust declaration, isolated install/uninstall smoke, metadata, docs, and deferral policy are complete. Marketplace/Open VSX submission and accepted listing remain explicitly deferred pending demand and external credentials. |
| 78 arXiv upstream requirements reconnaissance | completed | Search and document arXiv/submit-ce and arXiv/arxiv-submission-core contribution, schema, testing, security, and submission requirements. Requirements matrix documented with handoff to Tracks 79/80. Upstream submission remains out of scope. |
| 79 arXiv submit-ce maturity hardening | completed | Hardened the active submit-ce adapter with fixture breadth, schema-drift snapshot, security/no-writeback boundaries, and read-only smoke evidence. Upstream submission and maintainer acceptance remain separately gated. |
| 80 arXiv submission-core maturity hardening | completed | Documented the legacy/inactive submission-core contract, migration mapping, security/no-writeback boundaries, maintainer redirect, and terminal contracted evidence level. No active integration or acceptance is claimed. |
| 81 arXiv upstream submission and acceptance | completed | Create and track arXiv upstream issues/PRs only after requirements and hardening gates pass and approval is recorded. Issues filed: submit-ce #72 (open), submission-core #88 (closed/legacy). Evidence level is submitted; upstream acceptance remains unclaimed. |
| 82 self-improving submission and health loop | completed | Keep submission readiness machine-readable, workflow-checked, and tied to a repo-health target of at least 9.5 before external claims. Inventory schema, readiness script, and policy test are complete at fixture-backed evidence level; external acceptance remains separately bounded. |
| 83 Janeway platform reconnaissance | completed | Mapped official Janeway plugin, installation, typesetting, and API surfaces; selected a sidecar-first reconnaissance contract with explicit packaging, smoke, human-review, and no-support gates. |
| 84 Janeway plugin package hardening | completed | Added a preview-only Janeway adapter scaffold, explicit permissions, `janeway` platform wiring, deterministic fixture checks, and a host-namespaced archive; live install and smoke remain Track 85 gates. |
| 85 Janeway install and smoke evidence | completed | Recorded a fail-closed Janeway smoke harness and precise Docker/Podman runtime blocker; no installation, support, or publication claim is made. |
| 86 Proprietary journal-platform matrix | completed | Map ScholarOne, Editorial Manager, eJournalPress, and Manuscript Manager integration patterns, API access, and test environments. |
| 87 Self-improving platform registry | completed | Build the agentic registry, skill/workflow hooks, and track-synthesis loop that keeps platform evidence current. |
| 88 Open source documentation and maintenance hardening | completed | Audit and harden Sourceright public GitHub documentation, contributor experience, release hygiene, community-health files, and maintenance posture. |
| 89 Harness Engineering, Security, Quality, and CI/CD Hardening | completed | Harden repository automation, security, quality gates, cross-platform validation, and CI/CD evidence. |
| 90-citation Citation Ecosystem Differential Interoperability | completed | Compare and document citation ecosystem interoperability boundaries and fixture-backed differential evidence. |
| 91 CiteWeft Boundary and GROBID Reference Extraction | completed | Define the CiteWeft/GROBID-inspired extraction boundary and preserve canonical bibliographic-data ownership. |
| 92 Scholarly Extraction Benchmark | completed | Establish fixture-backed scholarly extraction benchmark contracts and reproducible regression evidence. |
| 93 Native PDF Layout and Token IR | completed | Add native PDF layout/token intermediate-representation contracts with deterministic evidence. |
| 94 Native Reference and Citation Model | completed | Define native scholarly reference and citation model boundaries while preserving CSL canonical data. |
| 95 Native Scholarly NER and Entity Linking | completed | Add native scholarly entity-linking and NER contracts with explicit confidence/provenance boundaries. |
| 96 Hybrid Extraction Routing and Production Hardening | completed | Harden hybrid native/CiteWeft extraction routing and production-facing validation contracts. |
| 97 CiteWeft Repository Extraction | completed | Preserve the extracted CiteWeft repository slice with destination approval, history, CI/security, stable package release, downstream smoke, and documented rollback/issue migration. |
| 88 Future scientific publishing and agentic editorial workflows | completed | Record the roadmap, workflow mappings, paper models, governance boundaries, and documentation for future scientific-publishing workflows. Implementation remains roadmap-level and does not claim autonomous editorial decisions. |
| 83 VS Code and Open VSX submission and acceptance | completed | VS Code extension package and Open VSX submission evidence are recorded; external acceptance remains separately bounded. |
| 84 Claude Desktop package submission and acceptance | completed | Claude Desktop/MCPB package evidence is recorded; external connector acceptance remains separately bounded. |
| 85 Codex app package submission and acceptance | completed | Codex app MCP configuration and package evidence are recorded; external acceptance remains separately bounded. |
| 86 GitHub Copilot extension submission and acceptance | completed | Copilot host-package evidence is recorded; external acceptance remains separately bounded. |
| 87 Gemini CLI extension submission and acceptance | completed | Gemini CLI extension evidence is recorded; external acceptance remains separately bounded. |
| 88 Qwen CLI extension submission and acceptance | completed | Qwen CLI extension evidence is recorded; external acceptance remains separately bounded. |
| 89 OpenCode plugin submission and acceptance | completed | OpenCode plugin/configuration evidence is recorded; external acceptance remains separately bounded. |
| 90 Cline MCP Marketplace submission and acceptance | completed | Cline marketplace package and issue-first submission evidence are recorded; external acceptance remains separately bounded. |
| 98 OJS workflow trial | completed | Trial Sourceright read-only fixture-backed metadata and report workflows against Open Journal Systems. Upstream tag/commit and installation blocker are recorded; GitHub issue #43; Project #17. |
| 99 Janeway workflow trial | completed | Extend the existing Janeway reconnaissance/package work into a read-only fixture-backed trial. Upstream tag/commit and installation blocker are recorded; GitHub issue #44; Project #17. |
| 100 Episciences workflow trial | completed | Trial overlay-journal metadata and provenance workflows against Episciences while preserving repository-source boundaries. GitHub issue #45; Project #17. |
| 101 Kotahi workflow trial | completed | Trial read-only journal/preprint workflow reporting against Kotahi with an explicit governance and project-transition gate. GitHub issue #46; Project #17. |
| 102 PubPub workflow trial | completed | Trial PubPub metadata and publication-report workflows through its documented API/import surface. GitHub issue #47; Project #17. |
| 103 OPS workflow trial | completed | Trial read-only preprint metadata/report workflows against PKP Open Preprint Systems using the shared PKP boundary. GitHub issue #48; Project #17. |
| 104 InvenioRDM workflow trial | completed | Trial REST/OAI metadata extraction and provenance reporting against an InvenioRDM instance. GitHub issue #49; Project #17. |
| 105 DSpace workflow trial | completed | Trial REST/OAI/SWORD metadata extraction and provenance reporting against DSpace. GitHub issue #50; Project #17. |
| 106 EPrints workflow trial | completed | Trial OAI/API metadata extraction and provenance reporting against EPrints. GitHub issue #51; Project #17. |
| 107 Samvera Hyrax workflow trial | completed | Trial Hyrax repository metadata extraction with deployment and community-contribution gates. GitHub issue #52; Project #17. |
...
| 109 Australian legal citation precision | completed | Improve full Australian neutral-citation extraction, prose rejection, provider hints, and explicit review confidence without changing the CSL boundary. GitHub issue #17. |
