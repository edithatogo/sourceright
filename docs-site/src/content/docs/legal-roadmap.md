---
title: Legal roadmap
description: Separate legal citation work and future legal-provider planning.
---

Legal citation work uses a separate model from academic CSL.

- Keep jurisdiction-specific logic separate from academic reference handling.
- Preserve the roadmap boundary between legal work and academic workflows.

The current legal model represents cases, legislation and regulations, neutral
citations, courts and jurisdictions, pinpoint references, public-provider
candidates, and legal review issues.

The first extractor detects neutral citations such as `[2016] HCA 1` and simple
legislation/regulation mentions. Provider slots are explicit for public sources
including CourtListener, Caselaw Access Project, AustLII, and legislation
registers. Use `sourceright legal <legal-text.txt>` to produce the baseline
legal citation report.

Subsequent treatment, reporter series, richer jurisdiction inference, and live
provider adapters remain later provider-expansion work.
