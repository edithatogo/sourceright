---
title: Guides
description: Operational guides for installing, running, and publishing Sourceright.
---

These pages describe the day-to-day operator surface:

- [Installation](installation)
- [Quickstart](quickstart)
- [Workflow](workflow)
- [Author Preflight Workflow](author-preflight-workflow)
- [Editorial Triage Workflow](editorial-triage-workflow)
- [University Repository Workflow](university-repository-workflow)
- [Legal Citation Mode Workflow](legal-citation-mode-workflow)
- [Legal Citation Audit MCP Pack](legal-citation-audit-mcp)
- [CLI](cli)
- [MCP](mcp)
- [Artifact and Schema Guide](artifact-schema-guide)
- [Live Provider Configuration](live-provider-configuration)
- [Publishing](publishing)
- [Limitations](limitations)
- [Benchmarks](benchmarks) - fixture-backed technical preview
- [Release Notes](release-notes)
- [FAQ](faq)
- [Pilot Invitation](pilot-invitation)
- [Release Status](../release-status)
- [Coverage Status](coverage-status)
- [Operations Status](operations-status)
- [Contributing](contributing)
