---
title: CiteWeft and optional GROBID extraction
description: Safe, review-first scholarly reference extraction boundaries.
---
...
The supported TEI subset preserves raw citations, TEI identifiers, authors,
...
fabricated. Typed failures cover no content, persistent overload, unsafe
endpoints, size limits, transport failures, HTTP failures, and malformed TEI.
...
`references.csl.json`. Default tests use self-authored TEI and require no
network, container, credentials, model download, or private document.

GROBID-NER is not a runtime dependency. Entity recognition remains independent
and outside CSL. Data, weights, taxonomy mappings, and compatibility bridges
require file-level provenance, licensing, version, and redistribution review.

Full-document extraction, coordinates, callouts, native PDF/layout, learned
reference parsing, NER models, hybrid routing, production hardening, and a
separate repository remain planned follow-on work.
